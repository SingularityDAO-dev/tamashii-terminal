...
export const runFeeTokenSelector = async (
  ...
  const additionalChoices = currentBroadcaster
    ? [
        ...
        {
          name: "list-all-broadcasters",
          message: "List All Available Broadcasters".grey,
        },
        {
          name: "view-broadcaster-info",
          message: "View Broadcaster Details / Top Up Info".grey,
        },
        {
  ...
  if (feeOption) {
    ...
    switch (feeOption) {
      case "view-broadcaster-info": {
        if (currentBroadcaster) {
          console.log("\n" + "=".repeat(60).cyan);
          console.log("📡 Broadcaster Details".cyan.bold);
          console.log("=".repeat(60).cyan);
          console.log(`\nRailgun Address (0zk):`.yellow);
          console.log(`  ${currentBroadcaster.railgunAddress}`.green);
          console.log(`\nFee Token Address:`.yellow);
          console.log(`  ${currentBroadcaster.tokenAddress || "N/A"}`.green);
          if (currentBroadcaster.tokenFee) {
            console.log(`\nFee Details:`.yellow);
            console.log(`  Fee per unit gas: ${currentBroadcaster.tokenFee.feePerUnitGas?.toString() || "N/A"}`.green);
            console.log(`  Fees ID: ${currentBroadcaster.tokenFee.feesID || "N/A"}`.green);
          }
          console.log("\n" + "=".repeat(60).yellow);
          console.log("💡 How to Top Up a Broadcaster".yellow.bold);
          console.log("=".repeat(60).yellow);
          console.log("Broadcasters are operated by third parties.");
          console.log("To top up a broadcaster, you need their PUBLIC address (0x...).");
          console.log("\nThe Railgun address shown above is their private address.");
          console.log("You cannot send BNB/ETH directly to a Railgun address.");
          console.log("\nOptions:".yellow);
          console.log("1. Contact the broadcaster operator to get their public address");
          console.log("2. Wait for the broadcaster to auto-refill (they usually do)");
          console.log("3. Try a different broadcaster");
          console.log("4. Use self-signing (if you have BNB for gas)");
          console.log("=".repeat(60).cyan + "\n");
          
          await confirmPromptCatchRetry("Press ENTER to continue...");
        }
        return runFeeTokenSelector(chainName, amountRecipients, currentBroadcaster);
      }
      case "list-all-broadcasters": {
        // Get the fee token first
        ...
        try {
          ...
          // Get ALL broadcasters for this token
          const allBroadcasters = await waku.findBroadcastersForToken(
            chain,
            feeTokenAddress.toLowerCase(),
            true,
          );
          
          if (!allBroadcasters || allBroadcasters.length === 0) {
            console.log("No Broadcasters Found for Token".yellow);
            return runFeeTokenSelector(chainName, amountRecipients, currentBroadcaster);
          }
          
          console.log(`\nFound ${allBroadcasters.length} broadcaster(s):`.green);
          
          // Create choices for all broadcasters
          const broadcasterChoices: { name: string; message: string }[] = allBroadcasters.map((b: SelectedBroadcaster, i: number) => {
            const shortAddr = `${b.railgunAddress.slice(0, 10)}...${b.railgunAddress.slice(-6)}`;
            const feeAmount = b.tokenFee?.feePerUnitGas ? 
              `Fee: ${b.tokenFee.feePerUnitGas.toString()}` : "";
            return {
              name: `broadcaster-${i}`,
              message: `${shortAddr} ${feeAmount}`.cyan,
            };
          });
          
          broadcasterChoices.push({ name: "go-back", message: "Cancel".grey });
          
          const broadcasterPrompt = new Select({
            header: " ",
            message: "Select a Broadcaster",
            choices: broadcasterChoices,
            multiple: false,
          });
          
          const selectedIndex = await broadcasterPrompt.run().catch(confirmPromptCatch);
          
          if (selectedIndex === "go-back" || !selectedIndex) {
            return runFeeTokenSelector(chainName, amountRecipients, currentBroadcaster);
          }
          
          // Extract the index from the selection
          const idx = parseInt(selectedIndex.replace("broadcaster-", ""));
          if (!isNaN(idx) && allBroadcasters[idx]) {
            return { bestBroadcaster: allBroadcasters[idx] };
          }
          
          return runFeeTokenSelector(chainName, amountRecipients, currentBroadcaster);
        } catch (err) {
          console.log("Error finding broadcasters:", err);
          return runFeeTokenSelector(chainName, amountRecipients, currentBroadcaster);
        }
      }
...
export const getTokenAmountSelectionPrompt = async (
  ...
  `;
  // Check if balance is effectively zero (might be a cache issue)
  const balanceIsZero = parseUnits(currentBalance, token.decimals) === 0n;
  
  const prompt = new Input({
    header: `\nHow much ${token.name.cyan} do you wish to transfer?\n${
      recipentString.cyan
    }Your Balance: ${currentBalance.toString().yellow}${balanceIsZero ? " (balance may still be loading)".grey : ""}\n`,
    message: `${token.symbol}:`,
    min: 0,
    initial: balanceIsZero ? "" : currentBalance, // Don't default to 0 if balance is zero
    // Don't set max if balance is zero - let user enter any amount
    // The transaction will fail later if they don't have enough
    result(value: string) {
      // If balance is zero (likely a cache issue), trust the user's input
      if (balanceIsZero) {
        return value;
      }
      // Otherwise clamp to balance
    ...
    for (const bal of balances) {
      ...
      while (!completed) {
        ...
        if (selection) {
          ...
          // Preserve selectedAmount - don't overwrite it with remaining balance
