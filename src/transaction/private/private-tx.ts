...
export const getOriginalGasDetailsForPrivateTransaction = async (
  ...
  try {
    const feeData = await getFeeDetailsForChain(chainName).catch((err) => {
      const errorMsg = (err as Error)?.message || String(err);
      throw new Error(`Failed to get gas fee details: ${errorMsg}`);
    }) as { gasPrice?: bigint; maxFeePerGas?: bigint; maxPriorityFeePerGas?: bigint };
