...
import { applyCustomBroadcasters } from "../waku/broadcaster-util";
...
export const initializeWalletSystems = async () => {
  ...
  // Set default network to BNB Chain (Binance)
  const currentNetwork =
    walletManager.keyChain.currentNetwork ?? NetworkName.BNBChain;
  
  // If no network is set in keychain, set it to BNB Chain
  if (!walletManager.keyChain.currentNetwork) {
    walletManager.keyChain.currentNetwork = NetworkName.BNBChain;
    const { keyChainPath } = configDefaults.engine;
    saveKeychainFile(walletManager.keyChain, keyChainPath);
  }
  ...
  initWakuClient()
    .then(async () => {
      ...
      // Apply any custom broadcasters configured by the user
