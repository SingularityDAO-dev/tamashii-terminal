...
export const updatePublicBalancesForChain = async (
  ...
  try {
    ...
    for (const index in addresses) {
      ...
      try {
        // Add timeout for each token balance fetch (10 seconds per token)
        const tokenInfoPromise = getTokenInfo(chainName, tokenAddress);
        const balancePromise = getERC20Balance(
          chainName,
          tokenAddress,
          public0XAddress,
        );
        ...
        const timeoutPromise = new Promise((_, reject) => {
          setTimeout(() => reject(new Error(`Timeout fetching balance for token ${tokenAddress}`)), 10000);
        });
        
        const [{ decimals }, amount] = await Promise.race([
          Promise.all([tokenInfoPromise, balancePromise]),
          timeoutPromise
        ]) as [any, any];
        ...
        const errorMsg = (tokenError as Error)?.message || String(tokenError);
        if (!errorMsg.includes("Timeout")) {
          console.error(`Warning: Failed to fetch balance for token ${tokenAddress}: ${errorMsg}`);
        }
    ...
    throw new Error(`Failed to update public balances for ${chainName}: ${errorMsg}`);
  }
};
...
export const getPrivateERC20BalanceForChain = (
  ...
  const token = cache[tokenAddress];
  if (token) {
  ...
  for (const cachedTokenAddress in cache) {
    if (cachedTokenAddress.toLowerCase() === tokenAddressLower) {
      return BigInt(cache[cachedTokenAddress].balance.amount);
    }
  }
