...
export const getWrappedTokenBalance = async (
  ...
  const balanceBucket = RailgunWalletBalanceBucket.Spendable;
  initPrivateBalanceCachesForChain(chainName, balanceBucket, railgunWalletID);
  
  let wrappedBalance = useGasBalance
    ? await getCurrentWalletGasBalance()
    : getPrivateERC20BalanceForChain(chainName, wrappedInfo.wrappedAddress);
  
  // If balance is 0 and we're getting private balance, try alternative methods
  if (!useGasBalance && wrappedBalance === 0n) {
    const cache = privateERC20BalanceCache[chain.type]?.[chain.id]?.[balanceBucket]?.[railgunWalletID];
    
    // Debug: Log cache structure to understand what's available
    if (cache) {
      const cacheKeys = Object.keys(cache);
      // Try exact match first
      const exactMatch = cache[wrappedInfo.wrappedAddress];
      if (exactMatch && exactMatch.balance) {
        wrappedBalance = BigInt(exactMatch.balance.amount);
      } else {
        // Try case-insensitive lookup
        const wrappedAddressLower = wrappedInfo.wrappedAddress.toLowerCase();
        for (const tokenAddress of cacheKeys) {
          if (tokenAddress.toLowerCase() === wrappedAddressLower) {
            const cachedBalance = cache[tokenAddress];
            if (cachedBalance && cachedBalance.balance) {
              wrappedBalance = BigInt(cachedBalance.balance.amount);
              break;
            }
          }
        }
    ...
    // Method 2: Try to get it from latest balance events and update cache
    if (wrappedBalance === 0n && walletManager.latestPrivateBalanceEvents) {
      for (const balanceEvent of walletManager.latestPrivateBalanceEvents) {
        if (balanceEvent.chain.id === chain.id && balanceEvent.railgunWalletID === railgunWalletID) {
          const tokenBalance = balanceEvent.erc20Amounts.find(
            (token) => token.tokenAddress.toLowerCase() === wrappedInfo.wrappedAddress.toLowerCase()
          );
          if (tokenBalance && tokenBalance.amount > 0n) {
            wrappedBalance = tokenBalance.amount;
            // Update the cache with this balance
            await updatePrivateBalancesForChain(chainName, balanceEvent).catch(() => {
